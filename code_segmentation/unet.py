...
from keras.layers import Input, concatenate, Conv2D, MaxPooling2D, Conv2DTranspose,Lambda,BatchNormalization, Dense, Flatten
...
#    inputs = Input((None, None, num_channel))
    ...
    conv_seg = Conv2D(1, 1, activation='sigmoid')(the_layer)
    ...
    #densen = Dense(num_class,activation='relu')(Flatten()(layer_set1[-1]))
    densen = Dense(num_class,activation='sigmoid')(Flatten()(convn))

    model = Model(inputs=[inputs], outputs=[conv_seg, densen])

    model.compile(optimizer=Adam(lr=the_lr,beta_1=0.9, beta_2=0.999,decay=1e-5), \
        loss=[crossentropy_cut,losses.mean_squared_error], \
